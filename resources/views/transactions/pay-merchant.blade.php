...
<style>
    ...
    .select2-container--default .select2-selection--single .select2-selection__rendered {
        ...
        padding-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}: 15px !important;
        padding-{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}: 40px !important;
        color: #333 !important;
    ...
    .select2-container--default .select2-selection--single .select2-selection__arrow {
        ...
        {{ app()->getLocale() == 'ar' ? 'left' : 'right' }}: 15px !important;
    }

    .select2-container--default .select2-selection--single .select2-selection__arrow b {
        border-color: #667eea transparent transparent transparent !important;
        border-width: 6px 5px 0 5px !important;
        margin-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}: -5px !important;
...
<div class="loading-overlay" id="pageLoadingOverlay">
    <div class="spinner-container">
        ...
        <p class="loading-text">{{ __('messages.loading') }}...</p>
    </div>
</div>
...
<div style="max-width: 750px; margin: 2rem auto;">
    <div class="card">
        ...
        <form method="POST" action="{{ route('pay.merchant') }}" id="paymentForm">
            @csrf
            ...
            <div class="form-group">
                <label for="merchant_select">
                    ...
                    {{ app()->getLocale() == 'ar' ? 'اختر التاجر' : 'Select Merchant' }}
                </label>
                <select name="merchant_id" id="merchant_select" class="form-control" required>
                    <option value="">
                        {{ app()->getLocale() == 'ar' ? 'ابحث عن التاجر...' : 'Search for merchant...' }}
                    </option>
                </select>
                @error('merchant_id')
            ...
            <div class="form-group">
                <label for="amount">
                    <span class="form-icon" style="color: #667eea;">💰</span>
                    {{ __('messages.amount') }} ({{ __('messages.currency_symbol') }})
                </label>
            ...
            <div class="form-group">
                <label for="description">
                    <span class="form-icon" style="color: #667eea;">📝</span>
                    {{ app()->getLocale() == 'ar' ? 'ماذا تشتري؟' : 'What are you buying?' }}
                </label>
                <input type="text" name="description" id="description" class="form-control"
                       placeholder="{{ app()->getLocale() == 'ar' ? 'مثال: شراء هاتف، فاتورة كهرباء، اشتراك شهري' : 'Example: Phone purchase, electricity bill, monthly subscription' }}"
                       required>
                @error('description')
                    <small style="color: #dc3545; margin-top: 5px; display: block;">{{ $message }}</small>
                @enderror
            ...
            <div class="form-group">
                <label for="pin">
                    <span class="form-icon" style="color: #667eea;">🔐</span>
                    {{ app()->getLocale() == 'ar' ? 'رقم PIN (4 أرقام)' : 'PIN Code (4 digits)' }}
                </label>
            ...
            <button type="submit" class="btn btn-primary btn-submit">
                <span style="font-size: 1.3rem; margin-{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}: 10px;">💳</span>
        ...
        <div class="balance-card">
            <div class="balance-info">
                <span class="balance-label">
                    <span style="font-size: 1.4rem; margin-{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}: 8px;">💵</span>
                    {{ __('messages.current_balance') }}:
                </span>
                <span class="balance-amount">
                    {{ __('messages.currency_symbol') }}{{ number_format(auth()->user()->balance, 2) }}
                </span>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#merchant_select').select2({
        ...
        placeholder: '{{ app()->getLocale() == "ar" ? "ابحث عن التاجر بالاسم أو النوع..." : "Search for merchant by name or type..." }}',
        ...
        language: {
            ...
            noResults: function() {
                return '{{ app()->getLocale() == "ar" ? "لم يتم العثور على نتائج" : "No results found" }}';
            },
            loadingMore: function() {
                return '{{ app()->getLocale() == "ar" ? "جاري تحميل المزيد..." : "Loading more..." }}';
            },
            inputTooShort: function() {
                return '{{ app()->getLocale() == "ar" ? "ابدأ الكتابة للبحث" : "Start typing to search" }}';
            }
        },
