...
<div class="edit-role-page">
    <div class="form-card">
        <h2>✏️ {{ __('messages.edit_role') }}: {{ $role->display_name }}</h2>

        @if($isSuperAdmin)
        <div class="warning-box">
            <h4>🔒 {{ __('messages.super_admin_protected') }}</h4>
            <p>{{ __('messages.super_admin_protection_message') }}</p>
        </div>
        @else
        <div class="info-box">
            ℹ️ {{ __('messages.role_info') }}:
            <strong>{{ $role->users->count() }}</strong> {{ __('messages.users_have_this_role') }}
        </div>
        @endif
        ...
        <form method="POST" action="{{ route('admin.roles.update', $role) }}">
            ...
            <div class="form-group">
                ...
                <small style="color: #f59e0b; margin-top: 0.5rem; display: block;">
                    🔒 {{ __('messages.cannot_change_super_admin_name') }}
                </small>
                @else
                <small style="color: #666; margin-top: 0.5rem; display: block;">
                    {{ __('messages.role_name_hint') }}
                </small>
                @endif
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="description"
                          ...
                          class="form-control"
                          rows="3"
                          placeholder="{{ __('messages.description') }}">{{ old('description', $role->description) }}</textarea>
            </div>

            <div class="permissions-section {{ $isSuperAdmin ? 'locked' : '' }}">
                @if($isSuperAdmin)
                    <h3 style="color: #667eea; margin-bottom: 1rem;">🔐 {{ __('messages.permissions') }}</h3>
                    <div class="super-admin-lock">
                        <h3>🔒</h3>
                        <p><strong>{{ __('messages.super_admin_has_all_permissions') }}</strong></p>
                        <p style="margin-top: 0.5rem;">{{ __('messages.super_admin_permissions_locked') }}</p>
                    </div>
                @else
                    <div class="permissions-header">
                        <h3>🔐 {{ __('messages.permissions') }}</h3>
                        <span class="permissions-counter">
                            <span id="selected-count">{{ count($rolePermissions) }}</span> / <span id="total-count">{{ $permissions->flatten()->count() }}</span> {{ __('messages.selected') }}
                        </span>
